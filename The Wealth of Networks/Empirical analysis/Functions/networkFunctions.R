...
coverage <-
  function(network,
    ...
    for (i in 1:nrow(setPS)) {
      ...
      row_sub = apply(cov, 1, function(row)
        all(row != 0))
      cov <- cov[row_sub,]
...
# Brokerage of node set
# `perCapita` generates the Brokerage measure
blockPower <-
  function(network,
           nodeNames,
           s,
...
coverageMeasure <-
  function(network,
    ...
    critsCov$coverageMeasure <-
      round(critsCov$coverage / critsCov$setSize,
            digits = 3)
