...
coverage <- function(network, nodeNames, s, adjMatrix, setPS, perCapita) {
  if (missing(s)) {
    s <- nrow(nodeNames) - 2
  }
...
criticalSets <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  if (missing(setPower)) {
  ...
  criticalSets <- subset(setPower, setPower$power > 0)
  return(criticalSets)
}
...
coverageMeasure <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  ...
  critsCov <- coverage(network, 
                       nodeNames, 
                       setPS = crits)
  critsCov$coverageMeasure <- round(critsCov$coverage/critsCov$setSize, digits = 3)
  return(critsCov)
}
...
nodeSetBrokerage <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  ...
  for (i in 1:nrow(nodeNames)) {
    s <- setBrokerage
    t <- sapply(1:nrow(s), function(x) i %in% s$set[[x]])
    s <- s[t, ]
...
nodeCriticality <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  ...
  for (i in 1:nrow(nodeNames)) {
    ...
    s <- s[t, ]
    if (nrow(s) > 0) {
      nodeCriticality[i] <- sum(s$criticalityMeasure)
    } else {
      nodeCriticality[i] <- 0
    }
  }
...
# \tilde(\beta)_i
nodeNormCriticality <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  critMeasure <- criticalityMeasure(network, 
                                    nodeNames)
  ...
  for (i in 1:nrow(nodeNames)) {
    ...
    s <- s[t, ]
    if (nrow(s) > 0) {
      nodeNormCriticality[i] <- sum(s$criticalityMeasure)
    } else {
...
# \bar(\gamma)_i
nodeCoverage <- function(network, nodeNames, s, adjMatrix, setPS, setPower) {
  ...
  for (i in 1:nrow(nodeNames)) {
    ...
    s <- s[t, ]
    if (nrow(s) > 0) {
      nodeNormCoverage[i] <- sum(s$coverageMeasure)
    } else {
